	   ...
	   LXI D,2794	;set DE
	   LXI H,3792	;set HL
	   SIM			;set inturrept mask as value of A
	   LXI SP,F001
	   CALL DEBUG
	   HLT
...
;problem, SP also has this function call stuff, so we need to remove it's info from DE 
;(DE = DE + 2 [remember SP stack works reverse that's why +])
	   INX D
	   INX D
;now store DE
	   CALL STORE
